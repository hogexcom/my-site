"""
基本解近似解法（MFS）によるViscous Fingering（Saffman-Taylor不安定性）のWeb版
"""
import numpy as np
import math
...
xlim = 4
ylim = 4
sigma = 1e-4  # 表面張力（小さいほど不安定）
myu = 1
b_2 = 12 * myu
N = 300  # 点の数
d = 0.3 / math.sqrt(N)  # 特異点の距離
q = -2.0  # 湧き出し強度（大きいほど膨張が速い）
A_dot = -q  # 面積変化率


def initial_data(N, reverse=False):
    """初期曲線データ（摂動付き円）"""
...
def G_matrix(x, y, z, N):
    ...
    norm_XY = np.linalg.norm(X - Y, axis=2)
    norm_XY = np.where(norm_XY < 1e-12, 1e-12, norm_XY)
    norm_XZ = np.linalg.norm(X - Z, axis=2)
    norm_XZ = np.where(norm_XZ < 1e-12, 1e-12, norm_XZ)
    G = 1.0 / (2 * math.pi) * np.log(norm_XY / norm_XZ)
    return G
...
def H_ji_matrix(x, y, z, N):
    ...
    dist_XY_sq = np.sum((X - Y) ** 2, axis=2)
    dist_XY_sq = np.where(dist_XY_sq < 1e-12, 1e-12, dist_XY_sq)
    dist_XZ_sq = np.sum((X - Z) ** 2, axis=2)
    dist_XZ_sq = np.where(dist_XZ_sq < 1e-12, 1e-12, dist_XZ_sq)
    return (
        1.0
        / (2 * math.pi)
        * (
            (X - Y) / dist_XY_sq[..., np.newaxis]
            - (X - Z) / dist_XZ_sq[..., np.newaxis]
        )
    )
...
def phi_hat(X_, t, curve):
    ...
    phi = sigma * k - q / (2 * math.pi) * np.log(np.linalg.norm(X_, axis=1))
    _phi_hat = np.concatenate([[A_dot], phi])
    return _phi_hat, phi
...
def v_step(curve, Q_hat, H_ji, X_, n):
    ...
    Q = Q_hat[1:]
    # 圧力勾配 + 原点からの湧き出し項
    P_nabra = np.sum(Q[..., np.newaxis, np.newaxis] * H_ji, axis=0) + q / (2 * math.pi) * (X_ / np.sum(X_ ** 2, axis=1)[..., np.newaxis])
    # v = ∇P · n （法線が内向きなので符号を反転して外向きに膨張）
    v = -1 * np.sum(P_nabra * n, axis=1)
...
def contour_data(y, z, Q_hat, xlim=4, ylim=4, resolution=30):
    """等高線データを計算"""
